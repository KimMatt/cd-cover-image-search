...
def searchTree(name, tree):
	...
	for fname, votes in finalAnswer.most_common(10):
		...
		answers.append((fname, inliers, matches, good, mask, 0))

	# Parameters
	a = 2.0
	b = 2.0
	# For eliminating answers with too few matches
	c = .5

	# Calculate scores for each
	for each in answers:
		# Trim those without enough matches
		if (len(each[2]) < (float(mostMatches) * c)):
			each[5] = 0
		each[5] = ((float(each[1]) / float(len(each[2]))) / a) + ((float(each[1]) / float(mostInliers)) / b )

	# Get answer with max score
	actualAnswer = max(answers,key=itemgetter(5))[0]
	finalGood = max(answers,key=itemgetter(5))[3]
	finalMask = max(answers,key=itemgetter(5))[4]
